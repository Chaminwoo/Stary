...
- **확인 없이 자유롭게 진행한다(사용자 전권 위임, 2026-06-28).** 파일 이동·생성·쓰기·편집, 빌드, 커밋, **푸시까지** 사용자에게 확인/허락을 받지 않는다. 작업을 끝내고 결과만 보고한다.
  - 아래 §1 의 "푸시 전 테스트 대기"·"커밋 함부로 금지"는 **이 규칙으로 대체됨** — 막힘 없이 끝까지 진행 후 보고.
  - 단, §0 의 절대 금지(원본 경로 수정·민감값 커밋·원본 Firebase 연결)만은 예외 없이 지킨다.
...
3. **빌드 성공 → 사용자 테스트 차례.** 여기서 멈춘다.
   - ⚠️ **사용자가 "테스트 완료"라고 명시하기 전에는 절대 `git push` 하지 않는다.** 커밋도 사용자 지시 없이는 함부로 하지 않는다.
