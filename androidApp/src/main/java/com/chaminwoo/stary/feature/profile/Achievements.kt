...
object Achievements {
    ...
    val titleAchievements: List<Achievement> = listOf(
        ...
        // 기록
        title("count_five", "손가락으로 셀 수 있는 밤", "다이어리 5개 작성하기") { it.diariesCreated >= 5 },
        title("photo_ten", "찰칵, 별 하나", "사진을 넣은 다이어리 10개 작성하기") { it.photoPosts >= 10 },
        title("video_first", "되감기 가능한 밤", "영상을 넣은 다이어리 작성하기") { it.videoPosts >= 1 },
        title("long_letter", "잉크가 모자란 밤", "본문 500자 이상인 다이어리 작성하기") { it.longPosts >= 1 },
        title("short_post", "세 줄 요약도 길다", "본문 20자 이하로 다이어리 작성하기") { it.shortPosts >= 1 },
        title("private_five", "서랍 속 은하", "'나만보기' 다이어리 5개 작성하기") { it.privatePosts >= 5 },
        // 꾸준함
        title("streak_three", "작심삼일 격파", "3일 연속 기록하기") { it.maxStreakDays >= 3 },
        title("streak_month", "달이 한 바퀴 도는 동안", "30일 연속 기록하기") { it.maxStreakDays >= 30 },
        title("days_hundred", "백 번째 밤의 축배", "서로 다른 100일에 기록하기") { it.distinctDays >= 100 },
        // 시간 · 특별한 날
        title("all_months", "달력을 한 바퀴", "1월부터 12월까지 모든 달에 기록하기") { it.distinctMonths >= 12 },
        title("new_year", "카운트다운 끝, 첫 별", "1월 1일에 기록하기", hidden = true) { it.newYearPost },
        title("christmas", "산타보다 먼저 온 별", "12월 24일이나 25일에 기록하기", hidden = true) { it.christmasPost },
        // 여행 · 장소
        title("places_five", "골목마다 별 하나", "서로 1km 이상 떨어진 5곳에서 기록하기") { it.distinctPlaces >= 5 },
        title("places_twenty", "지도 위 점 잇기", "서로 1km 이상 떨어진 20곳에서 기록하기") { it.distinctPlaces >= 20 },
        title("home_cluster", "내 구역 선포", "반경 1km 안에 내 별 5개 남기기") { it.maxLocalCluster >= 5 },
        title("revisit", "우리 전에 여기 왔었지", "같은 자리(50m 이내)에서 30일 넘게 지나 다시 기록하기", hidden = true) { it.revisitedSpot },
        // 열람 · 별 도감
        title("log_ten", "주머니가 반짝반짝", "별 도감에 다른 사람의 별 10개 모으기") { it.starLogCount >= 10 },
        title("log_hundred", "세상의 비밀 백 개", "별 도감에 다른 사람의 별 100개 모으기") { it.starLogCount >= 100 },
        title("neighbor", "옆집 별에게 인사", "다른 사람의 별 100m 안에 내 별 남기기") { it.nearNeighbor },
        // 사랑받기 · 친구
        title("like_ten_one", "두근, 열 번", "한 다이어리에 좋아요 10개 받기") { it.maxLikesOnOne >= 10 },
        title("views_one", "소문난 별", "한 다이어리 조회수 100 달성하기") { it.maxViewsOnOne >= 100 },
        title("comments_ten", "말 걸어주는 별", "댓글 10개 받기") { it.commentsReceived >= 10 },
        title("friend_first", "너랑 나랑 쌍둥이별", "친구 1명 만들기") { it.friends >= 1 },
        title("friends_ten", "열 명이 모이면 별자리", "친구 10명 만들기") { it.friends >= 10 },
        // 꾸미기
        title("shapes_five", "변신은 무죄", "서로 다른 별 모양 5개로 기록하기") { it.distinctShapes >= 5 },
        title("colors_eight", "팔레트를 엎질렀다", "서로 다른 별 색 8개로 기록하기") { it.distinctColors >= 8 },
    )
    ...
    val rewardAchievements: List<Achievement> = listOf(
        ...
        Achievement("shape_heart", "하트 부자", "좋아요 500개 받기", Reward.Shape(9)) { it.likesReceived >= 500 },
        Achievement("shape_comet", "시차 적응 중", "기록 두 곳이 5,000km 이상 떨어지기", Reward.Shape(10)) { it.maxSpanMeters >= 5_000_000f },
        Achievement("shape_snow", "아무도 밟지 않은 눈밭", "1km 안에 먼저 남겨진 별이 없던 곳에 별 3개 남기기", Reward.Shape(11)) { it.firstInArea >= 3 },
        Achievement("shape_sakura", "꽃비가 내리는 날", "하루에 별 3개 남기기", Reward.Shape(12)) { it.maxPostsInDay >= 3 },
        Achievement("shape_sun", "해보다 먼저 일어난 사람", "새벽 5~7시에 5번 기록하기", Reward.Shape(13)) { it.dawnPosts >= 5 },
        Achievement("shape_flame", "7일째 타오르는 중", "7일 연속 기록하기", Reward.Shape(14)) { it.maxStreakDays >= 7 },
        Achievement("shape_key", "열쇠 꾸러미가 무거워", "별 도감에 다른 사람의 별 30개 모으기", Reward.Shape(15)) { it.starLogCount >= 30 },
        Achievement("shape_clover", "소원 접수 완료", "11시 11분에 기록하기", Reward.Shape(16), hidden = true) { it.wishTimePost },
        Achievement(CROWN_ID, "왕관의 무게를 견뎌라", "일반 업적 30개 달성하기", Reward.Shape(17)) { it.achievementsDone >= CROWN_TARGET },
        Achievement("shape_galaxy", "은하가 나를 닮아간다", "다이어리 200개 작성하기", Reward.Shape(18)) { it.diariesCreated >= 200 },
        Achievement("shape_cat", "달빛에 홀린 고양이", "보름달 뜬 밤(저녁 6시~새벽 6시)에 기록하기", Reward.Shape(19), hidden = true) { it.fullMoonPost },
        Achievement("shape_plane", "우편함이 넘쳐요", "댓글 50개 받기", Reward.Shape(20)) { it.commentsReceived >= 50 },
        // ── 별 색 보상 (단색) ──
