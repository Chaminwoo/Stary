...
- [ ] **Bundle Identifier = `com.chaminwoo.stary.ios`** — Apple App ID 언더스코어(`_`) 불가로 `stary_ios` 에서 점 표기로 변경(2026-07-15).
...
- [x] Firebase 콘솔 → **`momentdiary-f26c8`** 프로젝트에 **iOS 앱 추가 완료** (Bundle ID `com.chaminwoo.stary.ios`, `GoogleService-Info.plist` 확보 → `iosApp/Sources/` 에 커밋됨).
...
- [ ] App ID 등록(`com.chaminwoo.stary.ios`) + Capabilities(Push Notifications, Sign in with Apple 등) 체크.
