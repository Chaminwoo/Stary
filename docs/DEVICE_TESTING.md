...
2. **App Store Connect**에서 앱 레코드 생성 — Bundle ID `com.chaminwoo.stary_ios`, 이름 Stary.
...
4. Firebase 콘솔(`momentdiary-f26c8`) → iOS 앱이 이미 등록돼 있으면 `GoogleService-Info.plist` 다운로드
   (없으면 Bundle ID `com.chaminwoo.stary_ios` 로 새 iOS 앱 추가 후 다운로드).
