...
## 8.62 해금 본문 가독성 보완 + 튜토리얼 문구 정리 (Android BUILD SUCCESSFUL 2026-09-23 · 기기 테스트 대기)

사용자: "해금 전에는 좋은데, 해금 이후 본문은 테두리를 바꾸니 가독성이 떨어져 — 지금 스타일 유지하며 보완해" +
"튜토리얼 별에 한 번 열어본 별은 어디서든 열 수 있다는 문구 추가".

### ① 왜 가독성이 떨어졌나 (8.60 의 부작용)
- 배경이 **순흑 `#0D0D0D`**, 글자가 `#F0F0F0` 이라 대비 자체는 19:1 로 충분하다. 문제는 대비가 아니라
  ⓐ 순흑 위 밝은 글자의 **할레이션**(글자가 번져 보임) ⓑ 옛 카드(`#CC14181C`)가 하던 **문단 경계**가 사라진 것.
  짧은 잠금 안내문에서는 안 드러나고 **긴 본문에서만** 드러난다("해금 전에는 좋은데" 와 정확히 일치).

### ① 보완 — 카드로 되돌리지 않고 면만 회복
- `DiaryLock.readingSurface()` 신설(internal): 테두리·둥근 모서리 **없이** `#15191F` 70% 를 깔고 위아래 18dp 페이드.
  좌우 끝이 십자 코너 지점과 맞물려 "십자가 이 면의 모서리"로 읽힌다.
  ⚠️ `Modifier` 순서는 **`readingSurface()` → `cornerCrossFrame()`** — 뒤집으면 면이 십자 팔을 덮는다
  (Compose draw 모디파이어는 체인 앞쪽이 아래).
- 타이포: 행간 26 → **28sp**, 자간 0.1, **`LineBreak.Paragraph`**(한글 기본은 글자 단위 줄바꿈이라 어절이 잘린다),
  글자 alpha **0.93**(할레이션 완화). 안쪽 여백 18 → 20/22dp.
- iOS `ReadingSurface`(DiaryLockViews.swift) 동일 수치. ⚠️ SwiftUI 는 **나중에 붙인 `.background` 가 더 뒤**라
  `.background(CornerCrossFrame(...)).background(ReadingSurface())` 순서라야 십자가 면 위에 온다(Android 와 반대로 쓴다).

### ② 튜토리얼 별
- 본문도 상세 화면과 **같은 형식**으로 교체(읽기 면 + 십자 코너, 옛 카드/테두리 삭제) — 첫 화면이 상세와 다르면 곧바로 이질감.
- 문구(`tutorial_star_msg` / `L10n.tutorialStarMsg`)는 **사용자가 직접 고친 한국어가 기준**, 3문단으로 정리:
  "이렇게 별을 누르면…" / **"한 번 열어본 별은 멀리 떨어져도 어디에서든 다시 열 수 있어요."** / "이제 우주에 이야기를 남겨보세요!"
  → en/ja + Android/iOS 4곳을 여기에 맞췄다.
- ⚠️ 이 정리로 **"열 수 있는 별은 100m 안에 있는 것뿐"** 문단과 "이 별은 안내용이라 사라져요" 문단이 빠졌다.
  100m 규칙은 잠금 화면(`detail_locked_title`)과 코치마크에서 다시 안내되지만, 튜토리얼에서 빠진 건 의도 확인 필요.

### 일일 알림 문구(8.61) 한국어 수정 반영
- 사용자가 한국어 5문장을 고쳤다(noon 4번, afternoon 3번, evening 6번, night 6번, late 6번) → en/ja + iOS 배열을 맞췄다.
- 검증 스크립트로 **30문장 × 3언어 = 90개 전부 Android == iOS 일치** 확인(Android XML 의 `'` 이스케이프만 차이).
...
## 8.68 유튜브 쇼츠용 시네마틱 광고 40초 (도구·영상만, 앱 코드 변경 없음 — 2026-09-25 · 사용자 검토 대기)
사용자: "앱 광고용 시네마틱 영상 30초~1분, 유튜브 쇼츠(최대 길이 확인), 딥한 설정 빼고, 예전 구상안 개선점 제시 후 만들어."
- **쇼츠 규격**: 최대 3분(2024-10 에 60초→3분), 세로·정사각형만 쇼츠로 분류. 1분 초과분은 Content ID 클레임 규칙 별도 → 1분 미만 + 앱 자체 BGM.
- **구안(15초 스토리보드) 대비 개선 → 사용자 선택**: 약 40초 · **앱 화면 아예 없음**(순수 시네마틱) · 클라이맥스 = 금빛 빛이 솟아 새 별 ·
  **무자막**(엔딩은 텍스트 없이 로고 이미지만). 설정 설명(“모든 별은 누군가의 기억”)·눈 매크로 전환·풍경 입자 해체는 뺐다.
- **결과물**: `references/stary_cinematic_9x16.mp4`(1080x1920 · 24fps · 40초 · H.264 CRF16 + AAC 192k · 약 65MB · 실측 −11.9 LUFS / TP −1.1dB — 쇼츠가 −14 로 자동 조정). **mp4 는 커밋하지 않는다**(대용량, `stary_ad_9x16.mp4` 와 같은 취급).
- **렌더러**: `tools/ad/cinematic.py`(numpy/PIL/scipy + ffmpeg, cv2 는 NumPy 2 와 충돌해 안 씀). 소스는 **git 이력에서 자동 추출**
  (`ab247e3^:references/stary 광고 씬 모음/` — 9/4 에 정리된 씬 스틸·Kling/Veo 생성 클립) → `build/cinematic/src` 캐시.
  `--stills 3 12 31` 로 특정 시각 프레임만 PNG(검수용, 12장 ≈ 40초). 전체 렌더 ≈ 16분(프레임당 ~1초). `--audio-only` = 영상 재사용, 소리만 다시 입힘.
- **컷 구성**: A 0–5 걷는 뒷모습(scene1.png 푸시인+걸음 흔들림) / B 4.6–9.6 멈춤·별이 하나 둘(scene1.mp4) / C 9.4–13.4 다섯 별로 팬(scene2.mp4) /
  D 13.4–16.4 금빛 별로 푸시인 → 방사형 금빛 / E 16.2–25.4 회상 4컷(노을 연인 6-1 · 벚꽃 클로즈업 · 불꽃놀이 가족 · 아빠→아이 팬 6-4) /
  F 25.2–40 다시 밤(scene2.png): 강물 일렁임, 29.6s 그의 몸 뒤(실루엣 마스크 `MAN_POLY`)에서 빛이 솟아 32.8s 4갈래 별로 피고 링 → 틸트업·풀백 → 35.6s 로고 → 페이드아웃.
- **16:9 → 9:16**: 가로를 넓게 자르고 위쪽을 같은 톤 밤하늘로 확장(`extend_sky` — 밝은 번짐은 위로 90px 에 소멸, 클립은 170줄 feather) +
  잔별(`StarField`)을 확장 하늘~원본 하늘에 이어 그려 경계를 숨김. 생성 클립의 워터마크(Kling 우하단, Veo 스파클)는 크롭 밖.
- **사운드**: BGM `bgm_forgotten_galaxy` 2:02.3~(가장 조용한 지점에서 시작 → 회상 진입 16s 에 빌드업, 클라이맥스 33s 에 최대) · 별이 뜰 때
  `sfx_spark_1~3` · 금빛 푸시인/빛이 솟을 때 합성 쉬머 · 별이 피는 순간 `sfx_star_birth`(화음 0.42s 정렬, 그 순간 BGM −6dB 덕킹 → 로고까지 −2dB) ·
