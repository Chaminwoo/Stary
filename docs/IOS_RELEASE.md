...
2. **App Store Connect 에서 앱 레코드 생성.**
   - Bundle ID: **`com.chaminwoo.stary.ios`** — ⚠️ Apple App ID 는 언더스코어(`_`) 불가라 `stary_ios` 를 못 쓴다(2026-07-15, 점 표기로 확정).
...
4. **Firebase iOS 앱 등록 + `GoogleService-Info.plist` 다운로드.**
   - Firebase 콘솔(`momentdiary-f26c8`) → iOS 앱 추가 → Bundle ID `com.chaminwoo.stary.ios` → `GoogleService-Info.plist` 다운로드.
