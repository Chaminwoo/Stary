...
- `TYPE_COUNT = 21` : 모양 수 — 0 4꼭지 스파클 / 1 5꼭지 별 / 2 6꼭지 / 3 8꼭지 가는 스파클 /
  4 다이아 스파클 / 5 꽃 / 6 다이아몬드 / 7 초승달 / 8 행성 (5~8은 수집 보상 형태) /
  **9 하트 / 10 혜성 / 11 눈꽃 / 12 벚꽃 / 13 태양 / 14 불꽃 / 15 열쇠 / 16 네잎클로버 / 17 왕관 / 18 나선 은하 /
  19 고양이 / 20 종이비행기**(2026-09-26 업적 보상). 9~20 은 `UnitKit`(0..1 좌표 부품 → 픽셀) + `union`/`minus`(Path.op)로
  만들고, iOS `StarShape.swift` 가 같은 좌표로 복제한다. 부품이 많아 비싸므로 `starPath` 는 **모든 타입을 크기 100 으로
  한 번 만들어 캐시(`pathCache`) 후 배율만** 준다(iOS 도 `StarShape.cache`). 구버전 앱은 모르는 타입을 8(행성)로 그린다.
...
- `starPath(type, sizePx)` : 모양 Path. **모양 추가/수정은 여기 + iOS `StarShape.swift` 동시에.**
  - 6 다이아몬드(결정): 2026-09 피드백으로 컷 라인 12개 → 거들 높이 중심(0.5, 0.40)에서 5갈래
    → 2026-09-22 "가운데 선 제거": 중심→컬릿 세로선 삭제, **크라운 V(테이블 양끝 → 중심) + 거들 가로선**만(💎 아이콘 형태).
    크기 `GEM_SCALE = 0.56`(0.66 → 0.56 "더 줄여" 피드백. 세로 중심 0.53→0.5 로 보정, 선 두께 0.03×배율),
