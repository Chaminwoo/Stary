...
- `focusDiary : DiaryFocusTarget?` : 외부 포커스 요청(lat/lng/색/id). `onFocusHandled()` : 소비 통지.
...
- 지도 클릭 리스너(1회 등록): 개척 비콘 탭→퀘스트 토스트, 별 탭→**거리 무관 진입**
  (효과음+스냅샷 → `warpState` 파장 → 상세).
  ⚠️ **100m 게이팅은 2026-09-21 에 지도에서 빠졌다.** 예전엔 여기서 거리 토스트로 진입을 막았지만,
  이제 누구나 들어가 **제목까지** 보고 미디어/본문/댓글만 상세 화면이 잠근다(06 문서 "열람 잠금", `DiaryLock.kt`).
- 스타일 초기화(`map.setStyle`): 레이어 순서(아래→위) =
  **별가루 파티클 → 별자리 3겹 → 바닥광 → 오오라 → 위성 → 별 → 스파클 → 개척 비콘 → 내 위치**.
...
- 포커스 `LaunchedEffect(focusDiary, mapRef)` : 대상 좌표로 800ms 카메라 → onFinish 에서 스냅샷 파장
  (`navigateAfter=false` — 지도에 머묾). 친구 행/프로필 핀 별/알림 **모두 같은 동작**.
...
- `STAR_SIZE_FAR = 0.65` / `STAR_SIZE_NEAR = 0.9` : 100m 밖/안 별 iconSize 기준(크기 힌트 — 진입은 항상 가능).
...
- 별자리: `CONSTELLATION_NEIGHBORS = 2`, 불투명도 3겹 = 0.18/0.42/0.95.
...
(도보 길찾기(ORS)/`partialRouteFrom`/경로 레이어는 2026-09-21 사용자 요청으로 **전면 삭제**.)
...
- 입력 프로퍼티가 곧 커맨드 채널: `diaries`/`userLocation`/`onTapStar`/`focusTarget`/
...
- `StyleFx` enum = **Android DiaryMapMarkers 상수 패리티 묶음**: particleCount 400/20km/시드 42,
  phaseGroups 4, 별자리 불투명도 0.18/0.42/0.95·이웃 2, 바닥광 0.30/+8.
- `didFinishLoading` : 파티클/별자리/후광 소스·레이어 1회 설치 + 트윙클 루프 시작.
