...
enum Achievements {
    ...
    static let titleAchievements: [Achievement] = [
        ...
        // 기록
        titleAch("count_five", "손가락으로 셀 수 있는 밤", "다이어리 5개 작성하기") { $0.diariesCreated >= 5 },
        titleAch("photo_ten", "찰칵, 별 하나", "사진을 넣은 다이어리 10개 작성하기") { $0.photoPosts >= 10 },
        titleAch("video_first", "되감기 가능한 밤", "영상을 넣은 다이어리 작성하기") { $0.videoPosts >= 1 },
        titleAch("long_letter", "잉크가 모자란 밤", "본문 500자 이상인 다이어리 작성하기") { $0.longPosts >= 1 },
        titleAch("short_post", "세 줄 요약도 길다", "본문 20자 이하로 다이어리 작성하기") { $0.shortPosts >= 1 },
        titleAch("private_five", "서랍 속 은하", "'나만보기' 다이어리 5개 작성하기") { $0.privatePosts >= 5 },
        // 꾸준함
        titleAch("streak_three", "작심삼일 격파", "3일 연속 기록하기") { $0.maxStreakDays >= 3 },
        titleAch("streak_month", "달이 한 바퀴 도는 동안", "30일 연속 기록하기") { $0.maxStreakDays >= 30 },
        titleAch("days_hundred", "백 번째 밤의 축배", "서로 다른 100일에 기록하기") { $0.distinctDays >= 100 },
        // 시간 · 특별한 날
        titleAch("all_months", "달력을 한 바퀴", "1월부터 12월까지 모든 달에 기록하기") { $0.distinctMonths >= 12 },
        titleAch("new_year", "카운트다운 끝, 첫 별", "1월 1일에 기록하기", hidden: true) { $0.newYearPost },
        titleAch("christmas", "산타보다 먼저 온 별", "12월 24일이나 25일에 기록하기", hidden: true) { $0.christmasPost },
        // 여행 · 장소
        titleAch("places_five", "골목마다 별 하나", "서로 1km 이상 떨어진 5곳에서 기록하기") { $0.distinctPlaces >= 5 },
        titleAch("places_twenty", "지도 위 점 잇기", "서로 1km 이상 떨어진 20곳에서 기록하기") { $0.distinctPlaces >= 20 },
        titleAch("home_cluster", "내 구역 선포", "반경 1km 안에 내 별 5개 남기기") { $0.maxLocalCluster >= 5 },
        titleAch("revisit", "우리 전에 여기 왔었지", "같은 자리(50m 이내)에서 30일 넘게 지나 다시 기록하기", hidden: true) { $0.revisitedSpot },
        // 열람 · 별 도감
        titleAch("log_ten", "주머니가 반짝반짝", "별 도감에 다른 사람의 별 10개 모으기") { $0.starLogCount >= 10 },
        titleAch("log_hundred", "세상의 비밀 백 개", "별 도감에 다른 사람의 별 100개 모으기") { $0.starLogCount >= 100 },
        titleAch("neighbor", "옆집 별에게 인사", "다른 사람의 별 100m 안에 내 별 남기기") { $0.nearNeighbor },
        // 사랑받기 · 친구
        titleAch("like_ten_one", "두근, 열 번", "한 다이어리에 좋아요 10개 받기") { $0.maxLikesOnOne >= 10 },
        titleAch("views_one", "소문난 별", "한 다이어리 조회수 100 달성하기") { $0.maxViewsOnOne >= 100 },
        titleAch("comments_ten", "말 걸어주는 별", "댓글 10개 받기") { $0.commentsReceived >= 10 },
        titleAch("friend_first", "너랑 나랑 쌍둥이별", "친구 1명 만들기") { $0.friends >= 1 },
        titleAch("friends_ten", "열 명이 모이면 별자리", "친구 10명 만들기") { $0.friends >= 10 },
        // 꾸미기
        titleAch("shapes_five", "변신은 무죄", "서로 다른 별 모양 5개로 기록하기") { $0.distinctShapes >= 5 },
        titleAch("colors_eight", "팔레트를 엎질렀다", "서로 다른 별 색 8개로 기록하기") { $0.distinctColors >= 8 },
    ]
    ...
    static let rewardAchievements: [Achievement] = [
        ...
        Achievement(id: "shape_heart", name: "하트 부자", condition: "좋아요 500개 받기", reward: .shape(9)) { $0.likesReceived >= 500 },
        Achievement(id: "shape_comet", name: "시차 적응 중", condition: "기록 두 곳이 5,000km 이상 떨어지기", reward: .shape(10)) { $0.maxSpanMeters >= 5_000_000 },
        Achievement(id: "shape_snow", name: "아무도 밟지 않은 눈밭", condition: "1km 안에 먼저 남겨진 별이 없던 곳에 별 3개 남기기", reward: .shape(11)) { $0.firstInArea >= 3 },
        Achievement(id: "shape_sakura", name: "꽃비가 내리는 날", condition: "하루에 별 3개 남기기", reward: .shape(12)) { $0.maxPostsInDay >= 3 },
        Achievement(id: "shape_sun", name: "해보다 먼저 일어난 사람", condition: "새벽 5~7시에 5번 기록하기", reward: .shape(13)) { $0.dawnPosts >= 5 },
        Achievement(id: "shape_flame", name: "7일째 타오르는 중", condition: "7일 연속 기록하기", reward: .shape(14)) { $0.maxStreakDays >= 7 },
        Achievement(id: "shape_key", name: "열쇠 꾸러미가 무거워", condition: "별 도감에 다른 사람의 별 30개 모으기", reward: .shape(15)) { $0.starLogCount >= 30 },
        Achievement(id: "shape_clover", name: "소원 접수 완료", condition: "11시 11분에 기록하기", reward: .shape(16), hidden: true) { $0.wishTimePost },
        Achievement(id: Achievements.crownId, name: "왕관의 무게를 견뎌라", condition: "일반 업적 30개 달성하기", reward: .shape(17)) { $0.achievementsDone >= Achievements.crownTarget },
        Achievement(id: "shape_galaxy", name: "은하가 나를 닮아간다", condition: "다이어리 200개 작성하기", reward: .shape(18)) { $0.diariesCreated >= 200 },
        Achievement(id: "shape_cat", name: "달빛에 홀린 고양이", condition: "보름달 뜬 밤(저녁 6시~새벽 6시)에 기록하기", reward: .shape(19), hidden: true) { $0.fullMoonPost },
        Achievement(id: "shape_plane", name: "우편함이 넘쳐요", condition: "댓글 50개 받기", reward: .shape(20)) { $0.commentsReceived >= 50 },
        // 색 보상(단색)
